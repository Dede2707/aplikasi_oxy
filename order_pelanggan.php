...
// =========================================================================
// MANDIRI: PROSES KIRIM KOMPLAIN LANGSUNG (TANPA HARUS ORDER)
// =========================================================================
if (isset($_POST['btn_kirim_komplain_langsung'])) {
    $nama_kp     = mysqli_real_escape_string($koneksi, $_POST['kp_nama']);
    $wa_kp       = mysqli_real_escape_string($koneksi, $_POST['kp_wa']);
    $qty_kp      = !empty($_POST['kp_qty']) ? (int)$_POST['kp_qty'] : 0;
    $isi_keluhan = mysqli_real_escape_string($koneksi, $_POST['kp_pesan']);

    // Deteksi kolom tabel retur secara dinamis
    ...
    // Ambil ID Pelanggan & Barang pertama yang ada di DB hanya untuk bypass validasi FK
    $id_plg_fk = 1;
    $ambil_p = mysqli_query($koneksi, "SELECT id_pelanggan FROM pelanggan LIMIT 1");
    if ($ambil_p && mysqli_num_rows($ambil_p) > 0) {
        $res_p = mysqli_fetch_assoc($ambil_p);
        $id_plg_fk = $res_p['id_pelanggan'];
    }
    ...
    $ambil_b = mysqli_query($koneksi, "SELECT $pk_b FROM $tabel_b LIMIT 1");
    if ($ambil_b && mysqli_num_rows($ambil_b) > 0) {
        $res_b = mysqli_fetch_assoc($ambil_b);
        $id_brg_fk = $res_b[$pk_b];
    }

    // Format pesan komplain mandiri agar Admin mudah membaca data pengirim
    $format_komplain = "Kategori: Keluhan Mandiri Pelanggan\n" .
        "Pengirim: " . $nama_kp . " (WA: " . $wa_kp . ")\n" .
        "Detail Masalah / Riwayat Item:\n" . $isi_keluhan;

    $format_komplain = mysqli_real_escape_string($koneksi, $format_komplain);
    $tgl_skrg = date('Y-m-d');

    $q_komplain = "INSERT INTO retur ($col_tgl, $col_jml, id_pelanggan, id_barang, id_rekening, $col_ket) 
                   VALUES ('$tgl_skrg', $qty_kp, $id_plg_fk, $id_brg_fk, NULL, '$format_komplain')";

    if (mysqli_query($koneksi, $q_komplain)) {
        echo "<script>alert('✅ Laporan komplain Anda sudah terkirim ke Admin. Terima kasih!'); window.location.href='';</script>";
        exit();
    } else {
        $pesan_gagal = "Gagal mengirim keluhan: " . mysqli_error($koneksi);
    }
}

// =========================================================================
// SAKLAR 1: PROSES CHECKOUT ORDER BELANJA
// =========================================================================
...
// =========================================================================
// SAKLAR 2: KONFIRMASI PEMBAYARAN FINAL ORDER BELANJA
// =========================================================================
...
<head>
    ...
    <title>Oxywater - Portal Pelanggan</title>
...
<body>
        ...
        <?php if ($tampilan == "form_input"): ?>
            <div class="row justify-content-center mb-4">
                <div class="col-12 col-lg-7 text-center">
                    <ul class="nav nav-pills justify-content-center d-inline-flex p-2 bg-white rounded shadow-sm" id="menuTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active px-4 py-2" id="tab-order-btn" data-bs-toggle="pill" data-bs-target="#kotak-order" type="button" role="tab" aria-controls="kotak-order" aria-selected="true"><i class="fa-solid fa-cart-shopping me-2"></i>Belanja / Order</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link px-4 py-2 text-danger" id="tab-komplain-btn" data-bs-toggle="pill" data-bs-target="#kotak-komplain" type="button" role="tab" aria-controls="kotak-komplain" aria-selected="false"><i class="fa-solid fa-triangle-exclamation me-2"></i>Komplain & Keluhan</button>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="row justify-content-center g-4">
            ...
            <?php if (!empty($pesan_gagal)): ?>
                <div class="col-12 col-lg-7">
                    <div class="alert alert-danger mb-2"><?= $pesan_gagal ?></div>
                </div>
            <?php endif; ?>

            <?php if ($tampilan == "form_input"): ?>
                <div class="col-12 col-lg-7 tab-content" id="menuTabContent">

                    <div class="tab-pane fade show active" id="kotak-order" role="tabpanel" aria-labelledby="tab-order-btn">
                        <div class="card card-custom mb-4 bg-white">
                            <div class="brand-header text-center">
                                ...
                                <p class="mb-0 small opacity-75">Silakan masukan data dan pilih varian belanja Anda</p>
                            </div>
                            <div class="card-body p-4 text-dark">
                                <form action="" method="POST" id="formOrderCustomer">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" id="order_nama" name="nama_pelanggan" class="form-control" placeholder="Masukkan nama toko/pemesan" required>
                                    </div>
                                    <div class="row g-2 mb-4">
                                        <div class="col-md-6">
                                            <label class="form-label small fw-semibold text-muted">Nomor WhatsApp</label>
                                            <input type="tel" id="order_wa" name="no_telp" class="form-control" placeholder="08123xxx" required>
                                        </div>
                                        <div class="col-md-6">
                                    ...
                                    <div class="table-responsive bg-light p-2 rounded border mb-2">
                                    ...
                                    <div class="text-end mb-4">
                                        <button type="button" id="btnKomplainKeranjang" class="btn btn-sm btn-outline-danger fw-semibold" style="display:none;" onclick="pindahkanKeranjangKeKomplain()">
                                            <i class="fa-solid fa-file-circle-exclamation me-1"></i> Komplainkan Item di Keranjang Ini
                                        </button>
                                    </div>
                    ...
                    <div class="tab-pane fade" id="kotak-komplain" role="tabpanel" aria-labelledby="tab-komplain-btn">
                        <div class="card card-custom mb-4 bg-white border border-danger">
                            <div class="brand-header-komplain text-center">
                                <h3 class="fw-bold mb-1"><i class="fa-solid fa-triangle-exclamation me-2"></i>Pusat Keluhan & Komplain</h3>
                                <p class="mb-0 small opacity-75">Ada kendala pengiriman atau barang cacat? Laporkan di bawah</p>
                            </div>
                            <div class="card-body p-4 text-dark">
                                <form action="" method="POST">
                                    <div class="mb-3">
                                        <label class="form-label small fw-semibold text-secondary">Nama Anda / Nama Toko</label>
                                        <input type="text" id="kp_nama" name="kp_nama" class="form-control" placeholder="Masukkan nama Anda" required>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label small fw-semibold text-secondary">No. Handphone / WhatsApp</label>
                                        <input type="tel" id="kp_wa" name="kp_wa" class="form-control" placeholder="Contoh: 0812345678" required>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label small fw-semibold text-secondary">Total Qty Bermasalah (Dus)</label>
                                        <input type="number" id="kp_qty" name="kp_qty" class="form-control" value="0" min="0">
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label small fw-semibold text-secondary">Rincian Komplain & Detail Riwayat Barang</label>
                                        <textarea id="kp_pesan" name="kp_pesan" rows="6" class="form-control" placeholder="Tulis rincian keluhan Anda atau riwayat item di sini dengan jelas..." required></textarea>
                                    </div>

                                    <button type="submit" name="btn_kirim_komplain_langsung" class="btn btn-danger w-100 fw-bold py-3 shadow-sm">
                                        <i class="fa-solid fa-paper-plane me-2"></i>Kirim Laporan Komplain Sekarang
                                    </button>
                                </form>
                            </div>
                        </div>
                ...
                <div class="col-12 col-md-8 col-lg-6">
                    <div class="card card-custom bg-white shadow-lg">
                        <div class="card-header bg-warning text-dark text-center py-3 fw-bold fs-5" style="border-radius: 15px 15px 0 0;">
                            <i class="fa-solid fa-hourglass-half me-2"></i>Menunggu Pembayaran Anda
                        </div>
    ...
    <script>
        ...
        const btnKomplainKeranjang = document.getElementById('btnKomplainKeranjang');

        // FUNGSI UTAMA: MEMINDAHKAN DETAIL BARANG DI KERANJANG KE FORM KOMPLAIN
        function pindahkanKeranjangKeKomplain() {
            if (keranjang.length === 0) return;

            let teksRiwayat = "[Riwayat Item dari Keranjang]:\n";
            let totalQtyCacat = 0;

            keranjang.forEach((item) => {
                teksRiwayat += `- ${item.produk} sejumlah ${item.qty} Dus\n`;
                totalQtyCacat += item.qty;
            });
            teksRiwayat += "\nKendala/Masalah: (Silakan tulis kendala barangnya di sini...)";

            // Salin value dari form order ke form komplain otomatis
            document.getElementById('kp_nama').value = document.getElementById('order_nama').value;
            document.getElementById('kp_wa').value = document.getElementById('order_wa').value;
            document.getElementById('kp_qty').value = totalQtyCacat;
            document.getElementById('kp_pesan').value = teksRiwayat;

            // Trigger perpindahan Tab via JS Bootstrap 
            const tabKomplainTrigger = new bootstrap.Tab(document.getElementById('tab-komplain-btn'));
            tabKomplainTrigger.show();

            // Fokus langsung ke input keluhan agar user tinggal mengetik keluhannya
            setTimeout(() => {
                document.getElementById('kp_pesan').focus();
            }, 300);
        }
        ...
        function perbaruiTampilanKeranjang() {
            ...
            if (keranjang.length === 0) {
                ...
                if (btnKomplainKeranjang) btnKomplainKeranjang.style.display = "none";
                return;
            }
            ...
            if (btnKomplainKeranjang) btnKomplainKeranjang.style.display = "inline-block";
        }
    </script>
</body>
